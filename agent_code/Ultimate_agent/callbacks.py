...
def setup(self):
    ...
    if self.train:
        ...
        if not os.path.isfile("my-saved-model.pt"):
            ...
            self.model2.load_state_dict(torch.load('my-saved-model2.pt', map_location=torch.device('cpu')))
        self.model.to(self.model.device)
        self.model2.to(self.model.device)
...
def act(self, game_state: dict) -> str:
    ...
    b = self.model2.forward(input)[0]


    res = ACTIONS[(a+0.2*b).argmax()]
    return res
